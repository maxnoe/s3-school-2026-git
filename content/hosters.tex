...
\begin{frame}[c]{Git Hosting Providers}
  ...
  \begin{columns}[b, onlytextwidth]%
    \begin{column}{0.33\textwidth}%
      \centering%
      \href{https://github.com}{\includegraphics[width=\linewidth, height=0.25\textheight, keepaspectratio]{build/figures/github.png}}%
    \end{column}%
    \begin{column}{0.33\textwidth}%
      \centering%
      \href{https://gitlab.com}{\includegraphics[width=\linewidth, height=0.25\textheight, keepaspectratio]{build/figures/gitlab.png}}%
    \end{column}%
    \begin{column}{0.33\textwidth}%
      \centering%
      \href{https://codeberg.org}{\includegraphics[width=\linewidth, height=0.25\textheight, keepaspectratio]{build/figures/codeberg.pdf}}%
    \end{column}%
  \end{columns}
  \begin{columns}[t, onlytextwidth]%
    ...
    \begin{column}{0.33\textwidth}%
      \begin{itemize}
        \item Run by German non-profit
        \item Based on Open-Source Forgejo, lightweight solution for self-hosting:\\
          \href{htts://forgejo.org}{\includegraphics[width=\linewidth]{build/figures/forgejo.pdf}}
      \end{itemize}
    \end{column}%
  \end{columns}
\end{frame}
  ...
  \begin{description}[HTTPS]
    \item[HTTPS]
      \begin{itemize}
        ...
        \item Use GitHub's CLI tool or the git credential helper to have to enter credentials every time:\\
...
\begin{frame}[fragile]{Using ssh}
  ...
  \begin{enumerate}
    ...
    \item Use the ssh URL for cloning. To change the URL of an already cloned repository, do
      \begin{code}{bash}
        $ git remote set-url origin git@github.com:s3-school/s3-2026-lectures
      \end{code}
  \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]{Forks}
  ...
  \begin{itemize}
    \item Go to \url{https://github.com/s3-school/pkoffee}
    \item Click the \enquote{Fork} button
    \item Clone your fork, e.g. for me it will be:
      \begin{code}{bash}
        $ git clone git@github.com:maxnoe/pkoffee
      \end{code}
    \item Add the main repository as second remote. The name \enquote{upstream} is convention.
      \begin{code}{bash}
        $ git remote add upstream git@github.com:s3-school/pkoffee
      \end{code}
...
\begin{frame}[c, fragile]{Making a Pull Request from a forked Repository}
  \begin{itemize}
    \item You should never work on the main branch of a fork, as it would diverge from the main branch of the parent project
    \item To enforce this is, you can point the main branch of your local copy to upstream:
      \begin{code}{bash}
        $ git switch main
        $ git pull --set-upstream upstream main
      \end{code}
      Then, git will pull from upstream on main and you won't be able to push.

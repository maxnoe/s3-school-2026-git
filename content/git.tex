...
\begin{frame}{Central Concept: Repository}
  ...
  \begin{tikzpicture}[
    ...
    \node (idx) [gitstep, fill=yellow!20, below=0.4cm of wd, visible on=<2->] {Staging};
    \node (hist) [gitstep, fill=green!20, below=0.4cm of idx, visible on=<3->] {History};
    \node [right=1cm of wd, text width=0.5\textwidth, align=flush left, visible on=<1->] {
      What actually is on disk in the current working directory. 
    };
    \node [right=1cm of idx, text width=0.5\textwidth, align=flush left, visible on=<2->] {
      Changes that are saved to go into the next commit.
    };
    \node [right=1cm of hist, text width=0.5\textwidth, align=flush left, visible on=<3->] {
  ...
  \begin{itemize}
    ...
    \item<2-> \textcolor{vertexDarkRed}{Branch}: A named pointer to a commit
      \begin{itemize}
        ...
        \item Default branch: usually \texttt{main} or \texttt{master}
  ...
  \begin{itemize}
    \item The first thing you should do after installing git is tell it who you are:
      \begin{code}[title={\textcolor{negative!60!white}{\faExclamationTriangle} Fill your own information! \textcolor{negative!60!white}{\faExclamationTriangle}}]{bash}
        $ git config --global user.name "Maximilian Linhoff"
        $ git config --global user.email "maximilian.linhoff@cta-observatory.org"
      \end{code}
    ...
    \item \mintinline{bash}+git config --global+ modifies the users' global git config file in \mintinline{bash}+$HOME/.gitconfig+
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Exercise}
  \begin{itemize}
    \item Clone the repository with the school material: \url{https://github.com/s3-school/s3-2026-lectures/}
      \begin{visibleenv}<2->
        \begin{code}{bash}
          $ git clone https://github.com/s3-school/s3-2026-lectures/
        \end{code}
      \end{visibleenv}%
    ...
    \item Change into the new directory
      \begin{visibleenv}<2->
        ...
        \end{code}{bash}
      \end{visibleenv}%
    \item Create a new git repository in the new directory
      \begin{visibleenv}<2->
        \begin{code}{bash}
          $ git init
        \end{code}{bash}
...
\begin{frame}[c, fragile]{Accessing the log}
  \begin{itemize}
    ...
    \item With the oneline option, it becomes the nice mnemonic \enquote{a dog} \faDog:
      \begin{code}{bash}
...
\begin{frame}[fragile]{A note on colors}
  \begin{itemize}
    \item By default, git is using red/green for changes and in status
    \item This might be hard to differentiate if you have a Color Vision Deficiency
    \item You can configure git's color scheme, e.g. changing green to blue:
      \begin{code}{bash}
        $ git config --global color.diff.new "blue"
        $ git config --global color.status.added "blue"
      \end{code}
    \item There are more color options, but these two are the most important
  \end{itemize}
\end{frame}
...
\begin{frame}{Remotes}
  ...
  \end{tikzpicture}
  \end{center}
  More on different hosting providers (Software Forges) later
\end{frame}
...
\begin{frame}[fragile]{Syncing with remotes}
  \begin{itemize}
    ...
    \item Set the default remote for branch and push (needed when pushing a new branch for the first time)
      ...
      \end{code}
    \item Recent versions of git have a configuration option that make this unnecessary:
      \begin{code}{bash}
        $ git config --global push.autoSetupRemote true
      \end{code}
      With this, git push on a new branch will \enquote{just work}
  \end{itemize}
\end{frame}
...
\begin{frame}[c, fragile]{Default branch name}
  ...
  \begin{itemize}
    \item Some time ago, a political correctness discussion started around master/slave terminology in tech
    ...
    \item To use \texttt{main} locally (when using \mintinline{bash}+git init+)
      \begin{code}{bash}
...
\begin{frame}[fragile]{Beware: Merge conflicts}
  ...
  \begin{enumerate}
    ...
    \item Commit merged changes:
      ...
      \end{code}
  \end{enumerate}
  useful: \texttt{git config --global merge.conflictstyle zdiff3}
\end{frame}

\begin{frame}[t]{Relevant XKCD}%
  \centering%
  \includegraphics[height=0.95\textheight]{build/figures/xkcd_git.png}%

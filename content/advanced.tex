...
\begin{frame}[c, fragile]{Using git bisect for finding commits}
  \begin{itemize}
    ...
    \item Example using astropy, investigating \href{https://github.com/astropy/astropy/issues/18866}{astropy/astropy\#18866}
    \begin{code}{text}
      # start bisecting
      git bisect start
      # first checkout and
      git bisect bad v7.2.0rc1
      git bisect good v7.1.1
...
\begin{frame}[c]{Disclaimers}
  \begin{itemize}
    ...
    \item Maintainers of many large projects will even ask you to cleanup the history of your Pull Request
    \item Modifying already-pushed commits requires pushing with the \mintinline{bash}+--force+ option
    \item The default branch and any release branches should be protected against force pushes\\
...
\begin{frame}[c, fragile]{Submodules}
  \begin{itemize}
    ...
    \item Cloning does not include submodules by default, needs
      ...
      \end{code}
    \item Update submodules (e.\,g.\ if changed on the remote or if forgotten during clone)
      \begin{code}{bash}
